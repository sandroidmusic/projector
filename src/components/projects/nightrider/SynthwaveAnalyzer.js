...
/**
 * Synthwave-like Audio Analyzer
 */
export default class SynthwaveAnalyzer extends AbstractScene {
  //---------------------------------------------------
  //
  //  Constructor
  //
  ...
  play() {
    ...
    tracker.jogWheel.fill.material = sun;
    this.createChildren();
  ...
  async setup() {
    ...
    tween = new Tween();
    sun = await ThreeMaterial.load.image('/assets/nightrider/synthwave-sun.png');
  }
  ...
  animate() {
    try {
      ...
      const jogWheel = Projector.globals.tracker.jogWheel;
      jogWheel.scale.x = 1;
      jogWheel.scale.y = 1;
      if (sunTween) {
        sunTween.kill();
      }
      sunTween = tween.to(jogWheel.scale, { x: 0.85, y: 0.85, duration: MidiController.beatLength });
